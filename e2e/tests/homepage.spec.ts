...
test.describe('Homepage', () => {
    ...
    test.skip('should display the login button when no user is logged in', async ({ page }) => {
        const login = page.getByRole('button', { name: /Login/i})
        await playwrightExpect(login).toBeVisible()
    })
    
    test.skip('should display the sign up button when no user is logged in', async ({ page }) => {
